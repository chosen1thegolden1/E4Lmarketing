...
2. `src/generate` — one `claude-opus-5` call, strict JSON schema → `DemoSpec { businessName, services[], persona, faqs[], demoCopy{} }` ✅ _(needs `LLM_API_KEY`)_
...
```
Requires `LLM_API_KEY` (generate) and network egress to the prospect's site (scrape).
