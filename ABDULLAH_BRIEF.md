...
| Create a Calendar for the "Game Plan Call" (20 minutes) and grab the public booking link | **You or Rozel** | ✅ done 2026-08-18 — `CS-Game-Plan-Call` (id `s5N1dGoEvdzJ2B0oU1DS`), 20 min, Mon–Fri 9–5 Pacific, assigned to Chosen. Link: `https://api.leadconnectorhq.com/widget/bookings/cs-game-plan-call` — already wired into every CTA in `docs/OUTREACH_COPY_BANK.md`. **The link is tied to the slug `cs-game-plan-call`, not the calendar name — never edit that slug.** Reassigning the calendar's team member silently overwrites its availability with that user's personal Work Hours; re-check the 9:00 AM start and the Pacific timezone after any assignee change. |
